...
const processTransactionLog = async (log) => {
    ...
    const isTxnExist = await Transaction.findOne({ txnHash: log.transactionHash });
    if (!isTxnExist) {
...
const processSaleLog = async (log) => {
    ...
    const data = ethers.utils.defaultAbiCoder.decode(['uint256', 'uint256', 'uint256', 'uint256', 'uint256', 'string', 'uint256', 'uint256', 'bool'], log.data);
    ...
    await Sale.findOneAndUpdate({ saleId: hexToSaleId, txnHash: log.transactionHash }, createSaleData, { new: true, upsert: true });
};

const processIcoFinalizedLog = async (log) => {
    const finalTokens = ethers.BigNumber.from(log.topics[1]).toString();
    await Token.updateOne({ tokenSymbol: "ECT" }, { finalTokensSold: finalTokens, icoFinalized: true });
    await Sale.updateMany({ active: true }, { active: false });
};
...
exports.createSaleCron = async () => {
    cron.schedule(CRON_INTERVAL, async () => {
        try {
            ...
            const fromBlock = (JSON.parse(await redisClient.get('sale_last_block')) + 1) || (latestBlock - BLOCK_RANGE);
            console.log('Processing Create Sale blocks from', fromBlock, 'to', latestBlock);
...
const processSaleFinalizeLog = async (log) => {
    const hexToSaleId = ethers.BigNumber.from(log.topics[1]).toString();
    await Sale.updateMany({ saleId: hexToSaleId }, { active: false, isFinalized: true });
};
...
exports.saleFinalizeCron = async () => {
    cron.schedule(CRON_INTERVAL, async () => {
        try {
            ...
            const latestBlock = await getLatestBlock();
            const fromBlock = (JSON.parse(await redisClient.get('sale_finalize_last_block')) + 1) || (latestBlock - BLOCK_RANGE);
            console.log('Processing Sale Finalize blocks from', fromBlock, 'to', latestBlock);
            ...
            console.log('Received log in Sale Finalize Cron:', saleFinalizeLogs.length);
...
exports.transactionCron = async () => {
    cron.schedule(CRON_INTERVAL, async () => {
        try {
            ...
            const latestBlock = await getLatestBlock();
            const fromBlock = (JSON.parse(await redisClient.get('transaction_last_block')) + 1) || (latestBlock - BLOCK_RANGE);
            console.log('Processing Transaction blocks from', fromBlock, 'to', latestBlock);
...
exports.icoFinalizedCron = async () => {
    cron.schedule(CRON_INTERVAL, async () => {
        try {
            ...
            const latestBlock = await getLatestBlock();
            const fromBlock = (JSON.parse(await redisClient.get('icofinalized_last_block')) + 1) || (latestBlock - BLOCK_RANGE);
            console.log('Processing ICO Finalized blocks from', fromBlock, 'to', latestBlock);
            ...
            console.log('Received log in ICO Finalized Cron:', icoFinalizedLogs.length);
...
const handleNormalizedUser = async (userAddress) => {
    try {
        await PrivateAddress.deleteOne({ walletAddress: { $regex: `^${userAddress}$`, $options: "i" } });
        await BlacklistAddress.deleteOne({ walletAddress: { $regex: `^${userAddress}$`, $options: "i" } });
    } catch (error) {
...
const handleWhitelist = async (userAddress) => {
    try {
        await PrivateAddress.updateOne({ walletAddress: { $regex: `^${userAddress}$`, $options: "i" } }, {
...
const handleBlacklist = async (userAddress) => {
    try {
        await BlacklistAddress.updateOne({ walletAddress: { $regex: `^${userAddress}$`, $options: "i" } }, {
            walletAddress: userAddress
        }, { upsert: true });
        await PrivateAddress.deleteOne({ walletAddress: { $regex: `^${userAddress}$`, $options: "i" } });
    } catch (error) {
...
exports.userStatusUpdateCron = async () => {
    cron.schedule(CRON_INTERVAL, async () => {
        try {
            ...
            const latestBlock = await getLatestBlock();
            const fromBlock = (JSON.parse(await redisClient.get('user_status_update_last_block')) + 1) || (latestBlock - BLOCK_RANGE);
            console.log('Processing User Status Update blocks from', fromBlock, 'to', latestBlock);
            ...
            console.log('Received log in User Status Update Cron:', userStatusUpdateLogs.length);
            ...
            for (const log of userStatusUpdateLogs) {
                ...
                await updateRedisBlock('user_status_update_last_block', log.blockNumber);
            }
        } catch (error) {
...
const processClaimTokenLog = async (log) => {
    ...
    if (data.length) {
        ...
        await TokenClaimHistory.updateOne({ txnHash: log.transactionHash }, claimTokenData, { upsert: true });
    }
};
...
exports.claimTokenCron = async () => {
    cron.schedule(CRON_INTERVAL, async () => {
        try {
            ...
            const latestBlock = await getLatestBlock();
            const fromBlock = (JSON.parse(await redisClient.get('claim_token_last_block')) + 1) || (latestBlock - BLOCK_RANGE);
            console.log('Processing claim Token blocks from', fromBlock, 'to', latestBlock);
