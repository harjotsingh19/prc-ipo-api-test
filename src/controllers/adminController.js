...
const { statusCode, message, status: userStatus, emailTemplateId } = require("../config/constants");
...
const setAdminAddress = async (req, res) => {
    try {
        ...
        const admin = await User.findById({ _id: new mongoose.Types.ObjectId(req.data.id) })
...
const getInvestors = async (req, res) => {
    try {
        ...
        if (req.query.investorId) {
            condition = { role: "INVESTOR", _id: new mongoose.Types.ObjectId(req.query.investorId) }
        }
        ...
        const paginatedData = (req.query.investorId && investors.length) ? investors : investors[0]?.data || [];
        const totalCount = (req.query.investorId && investors.length) ? 1 : investors[0]?.totalCount || 0;
...
const createSale = async (req, res) => {
    try {
        const { saleId, startTime, endTime, tokenPrice, txnHash, blockNumber, blockHash, txnIndex } = req.body;
        ...
        const sale = await Sale.create({ saleId, startTime, endTime, tokenPrice, txnHash, blockNumber, blockHash, txnIndex, active: true });
        return httpResponse(
            res,
            statusCode.ok,
            true,
...
const getInvestorKyc = async (req, res) => {
    try {
        ...
        if (req.query.investorId) {
            investorCondition = { role: "INVESTOR", _id: new mongoose.Types.ObjectId(req.query.investorId) }
        }
        ...
        const kycData = await User.aggregate([
                    ...
                    pipeline: [
                        {
                            $match: {
                                $expr: {
                                    $eq: [{ $toLower: "$walletAddress" }, "$$investorWalletAddress"]
                                }
                            }
                        }
                    ],
            ...
            {
                $addFields: {
                    isDocUploaded: { $cond: [{ $gt: [{ $size: "$kyc" }, 0] }, true, false] },
                    latestKyc: {
                        $ifNull: [
                            {
                                $arrayElemAt: [
...
const getAllAddressWhitelist = async (req, res) => {
    try {
        ...
        const whitelistAddressList = await PrivateAddress.find().sort({ createdAt: -1 }).skip(skip).limit(pageSize);
...
const getAllAddressBlacklist = async (req, res) => {
    try {
        ...
        const blacklistAddressList = await BlacklistAddress.find().sort({ createdAt: -1 }).skip(skip).limit(pageSize);
...
const getOneInvestorAllInvestments = async (req, res) => {
    try {
        ...
        const investor = await User.findOne({ walletAddress: { $regex: `^${walletAddress}$`, $options: "i" } });
        if (!investor) {
            return httpResponse(
                res,
                statusCode.badRequest,
        ...
        const isUserWhitelisted = await PrivateAddress.findOne({ walletAddress: { $regex: `^${investorDetails.walletAddress}$`, $options: "i" } });
        const isUserBlacklisted = await BlacklistAddress.findOne({ walletAddress: { $regex: `^${investorDetails.walletAddress}$`, $options: "i" } });
...
const getUserAnalytics = async (req, res) => {
    try {
        ...
        for (let i = 1; i <= saleCount; i++) {
            const vestingData = await vestingContract.methods.getVestingDetails(i, user.walletAddress).call();
            distributionAnalytics.totalClaimedTokens += Number(vestingData._claimedAmount);
            distributionAnalytics.totalTokens += Number(vestingData._totalAmount);
        }
        distributionAnalytics.totalUnclaimedTokens = distributionAnalytics.totalTokens - distributionAnalytics.totalClaimedTokens;
        responseData.distributionAnalytics = distributionAnalytics;
    
        if (userAnalytics.length) {
            responseData.groupedByTotalAssetAmount = userAnalytics[0]?.groupedByTotalAssetAmount || {};
...
const dashboard = async (req, res) => {
    try {
        ...
        const recentTransactions = await Transaction.find().sort({ created_at: -1 }).limit(10);
        const saleCount = Number(await rvaContract.methods.saleCount().call());
        const distributionAnalytics = {
            totalClaimedTokens: 0,
            totalUnclaimedTokens: 0,
        ...
        for (let i = 1; i <= saleCount; i++) {
            ...
            for (const investor of investors) {
                const vestingData = await vestingContract.methods.getVestingDetails(i, investor.investorAddress).call();
                distributionAnalytics.totalClaimedTokens += Number(vestingData._claimedAmount);
                distributionAnalytics.totalTokens += Number(vestingData._totalAmount);
            }
        }
    ...
    try {
        ...
        const tokenAnalytics = await Transaction.aggregate([{
            ...
            $group: {
                ...
                totalUnclaimedTokens: { $sum: "$tokenVestingsDetails.unclaimedTokens" },
        ...
        responseData.totalClaimedTokens = tokenAnalytics[0]?.totalClaimedTokens || 0;
        responseData.totalUnclaimedTokens = tokenAnalytics[0]?.totalUnclaimedTokens || 0;
...
const updateInvestorKycStatus = async (req, res) => {
    try {
        ...
        const statusHistory = { status: investor.status, timestamp: Date.now(), reason: "" };
        let isKycVerified;
        let isKycRejected;
        let reviewStatus;
        ...
        if (status === userStatus.KYC_SUCCESS) {
            ...
            if (isVerified) {
                await PrivateAddress.deleteOne({ walletAddress: { $regex: `^${investor.walletAddress}$`, $options: "i" } });
                await BlacklistAddress.deleteOne({ walletAddress: { $regex: `^${investor.walletAddress}$`, $options: "i" } });
            } else {
                return httpResponse(
                    res,
                    statusCode.badRequest,
        ...
        await User.updateOne({ _id: investorId }, { $push: { statusHistory: statusHistory }, $set: { status, isKycVerified, isKycRejected } });
        await Kyc.updateMany({ userId: investorId, isLatest: true }, { isKycVerified, rejectionReason, reviewStatus, lastUpdated: new Date() });
        return httpResponse(
            res,
            statusCode.ok,
            true,
            message.kycStatusUpdated,
            {},
        );
    } catch (error) {
        return httpResponse(
...
const getClaimTokenHistory = async (req, res) => {
    try {
        ...
        const claimTokenHistory = await TokenClaimHistory.find(query).sort({ createdAt: -1 }).skip(skip).limit(pageSize);
...
const downloadInvestments = async (req, res) => {
    try {
        ...
        const rows = transactions.map((transaction) => ({
            ...
            ['Tokens Bought With']:
              formatAssetAmount(transaction.assetAmount, transaction.asset),
            ['Token Amount']:
              transaction.tokenAmount / ECT_PRICE_MULTIPLIER,
            ['Token Price']:
              `$${transaction.tokenPrice / ECT_PRICE_MULTIPLIER}`,
            ['Time']:
            moment(transaction.created_at).format('DD/MM/YYYY, HH:mm:ss'),
        }));
        await Promise.all(rows);
        const pdfBuffer = generatePDF({ headers, rows, title: 'Investments Transaction' });

        res.setHeader("Content-Type", "application/pdf");
        res.setHeader("Content-Disposition", 'attachment; filename="InvestmentsTransactions.pdf"');
