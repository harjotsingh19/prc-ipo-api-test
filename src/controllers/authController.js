...
const { statusCode, message, status, otpOperations, emailTemplateId } = require("../config/constants");
...
const login = async (req, res) => {
    try {
        ...
        const accessToken = await jwtSign(userData, config.accessTokenSecret, config.accessTokenExpiry);
        const refreshToken = await jwtSign(userData, config.accessTokenSecret, config.refreshTokenExpiry);
...
const forgotPassword = async (req, res) => {
    try {
        ...
        await sendEmail(admin.email, emailTemplateId.adminResetPassword, {
            resetLink
        });
        return httpResponse(
            res,
...
const resetPassword = async (req, res) => {
    try {
        ...
        return httpResponse(
            ...
            statusCode.ok,
            true,
            message.credentialsResetSuccess,
        );
...
const addWalletAddress = async (req, res) => {
    try {
        ...
        const walletExists = await User.findOne({ walletAddress: { $regex: `^${walletAddress}$`, $options: "i" } });

        if (walletExists) {
            const accessToken = await jwtSign({ _id: walletExists._id, role: walletExists.role }, config.accessTokenSecret, config.accessTokenExpiry);
            const refreshToken = await jwtSign(walletExists, config.accessTokenSecret, config.refreshTokenExpiry);
        ...
        const statusHistory = { status: status.NEW, timestamp: Date.now(), reason: "" }

        const investor = await User.create({ walletAddress, role: "INVESTOR", isVerified: true, isKycVerified: false, isActive: true, status: status.NEW, statusHistory: statusHistory })
        const accessToken = await jwtSign(investor, config.accessTokenSecret, config.accessTokenExpiry);
        const refreshToken = await jwtSign(investor, config.accessTokenSecret, config.refreshTokenExpiry);
...
const refreshToken = async (req,res) => {
    try {
        ...
        const accessToken = await jwtSign(userData, config.accessTokenSecret, config.accessTokenExpiry);
        const newRefreshToken = await jwtSign(userData, config.accessTokenSecret, config.refreshTokenExpiry);
