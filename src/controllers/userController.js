...
const enableMFA = async (req, res) => {
    try {
        if (req.data.id && (req.data.id.toString() == req.params.id.toString())) {
            ...
            await User.findByIdAndUpdate(req.params.id, { mfaSecret: secretKey.ascii }, { new: true });
            const otpauth_url = speakeasy.otpauthURL({ secret: secretKey.ascii, label: "Security Code", algorithm: 'sha512' });            
